...
nu = [i for i in range(1, int(1.0e14), int(1.0e12)) ]

# Calculate the spectral radiance for each temperature
B = []
for temp in T:
    B_temp = [(2*h*nu[i-1]**3)/(c**2 * (math.exp((h*nu[i-1])/(k*temp)) - 1)) for i in range(1, len(nu)+1)]
    B.append(B_temp)
